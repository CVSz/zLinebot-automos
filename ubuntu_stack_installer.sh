#!/usr/bin/env bash
# zLineBot-automos Ubuntu Stack Installer - Project-root generator
# Target: Ubuntu 24.04 VM (VMware 16GB RAM / 300GB NVMe)
# Installs: Docker stack (API, Worker, Postgres, Redis, Kafka, NGINX panels)
# Usage:
#   sudo bash ubuntu_stack_installer.sh --domain zlinebot-automos.local [--app-dir /workspace/zLine/zlinebot-automos-stack]

set -euo pipefail

DOMAIN=""
CERT_EMAIL=""
APP_DIR="$(pwd)/zlinebot-automos-stack"
EXPORT_ZIP="false"
...
if [[ -z "$DOMAIN" ]]; then
  echo "Usage: sudo bash ubuntu_stack_installer.sh --domain your-domain [--cert-email admin@your-domain] [--export-zip] [--skip-full-pack] [--app-dir /path/to/output]"
  exit 1
fi
...
log "[3/8] Generate project files in $APP_DIR"
